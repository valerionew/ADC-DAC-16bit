...
class AD5667R{
  ...
  AD5667R(uint8_t i2cAddress=0x0F);
  ...
  uint8_t sendCommand(uint8_t selection, Command command, Dac dac, uint8_t high, uint8_t low, uint64_t timeout);
  
  uint8_t setReference(Dac dac, Reference ref, uint64_t timeout = 0){
    return sendCommand(0, Command::CMD_SETUP_REF, dac, 0, static_cast<int>(ref), timeout);
  }

  uint8_t setLdac(Dac dac, Ldac ldac, uint64_t timeout = 0){
    return sendCommand(0, Command::CMD_SETUP_LDAC, dac, 0, static_cast<int>(ldac), timeout);
  }

  uint8_t setOutput(Dac dac, uint16_t value, uint64_t timeout = 0){
